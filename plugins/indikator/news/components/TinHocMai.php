...
class TinHocMai extends ComponentBase
{
    public $posts;
    ...
    public function componentDetails()
    {
        return [
            'name'        => 'Tin HOCMAI',
            'description' => 'Danh sách tin học mãi'
        ];
    }
    ...
    public function onRun()
    {
        ...
        $this->postsUpcoming = $this->posts->where('featured',1)->take(5);
        $hotNews = $this->posts->where('hot_news',1);
        $this->firstPosts = $this->page['firstPosts'] = $hotNews->take(1);
        $this->twoPosts = $this->page['twoPosts'] = $hotNews->slice(1,2);
        // $this->firstPosts = $this->page['firstPosts'] = $this->posts->first();
        // $this->twoPosts = $this->page['twoPosts'] = $this->posts->slice(1,2);
        $this->postsNomal = $this->page['postsNomal'] = $this->posts->splice(3);

        $this->postReadMosts = $this->page['postReadMosts'] = NewsPost::where('category_id', 1)->orderBy('read_most', 'desc')
            ->take(10)
            ->get();
    }
