...
class Posts extends Model
{
    ...
    public $rules = [
        ...
        // , 'unique:indikator_news_posts'
        'status'   => 'required|between:1,3|numeric',
        'link_event'   => ['regex:/^(https?:\/\/)?([\da-z\.-]+)\.([a-z\.]{2,6})([\/\w \.-]*)*\/?$/'],
    ...
    public function scopeListFrontEndHm($query, $options)
    {
        extract(array_merge([
            'page'     => 1,
            'perPage'  => 10,
        ...
        return $query->where('category_id', $category)->paginate($perPage, $page);
    }

    public function scopeListFrontEndBc($query, $options)
    {
        extract(array_merge([
            'page'     => 1,
            'perPage'  => 10,
        ...
        return $query->where('category_id', 2)->paginate($perPage, $page);
    }
}
